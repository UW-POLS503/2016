...
In this lesson we will use the `Amelia` package and a subset of Beatrice Magistro's [dataset](https://www.dropbox.com/s/521gqn9neemmsd3/ess_sub.csv?dl=0), with data from the `European Social Survey`. Notice that for the `Zelig` package, we need an older version (`Zelig4`). This is because the new version runs into some issues when estimating models using multiple imputated datasets.
