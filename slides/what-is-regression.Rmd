...
- Each of these are conditional expectations. Define $Y$ to be the prestige of a profession, $X = ("bc", "wc", "prof")$
\[\begin{aligned}
\mu_{bc} & = E[Y|X = bc] \\
\mu_{wc} & = E[Y|X = wc] \\
\mu_{prof} &= E[Y | X = prof]
...
\[\begin{aligned}
\widehat{E}[Y_i|X_i = wc] &= \frac{1}{n_1} \sum_{i: X_i = wc} Y_i \\
\widehat{E}[Y_i|X_i = bc] &= \frac{1}{n_0} \sum_{i: X_i = bc} Y_i \\
\widehat{E}[Y_i|X_i = prof] &= \frac{1}{n_0} \sum_{i: X_i = prof} Y_i
\end{aligned}\]
